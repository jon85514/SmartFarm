...
public class SmartFarmDashboardController implements FlightAnimation {
	...
	public void visitItem(Item item) {
		...
		scanpath.getElements().add(new LineTo(item.getPosX(), item.getPosY()));
		scanpath.getElements().add(new VLineTo(item.getPosY() + item.getWidth()));
		scanpath.getElements().add(new HLineTo(item.getPosX() + item.getLength()));
		scanpath.getElements().add(new VLineTo(item.getPosY()));
		scanpath.getElements().add(new HLineTo(item.getPosX()));
		scanpath.getElements().add(new LineTo(item.getPosX() + item.getLength()/2, item.getPosY() + item.getWidth()/2));
		scanpath.getElements().add(new LineTo(xPos + 20, yPos + 20));
	...
	public void startFlight() {
		...
		if(VisitItemAction.isPressed())
			telloAdapter.visitItem(getSelectedItem());
		else if (ScanFarmAction.isPressed())
